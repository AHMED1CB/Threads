...
export default function Store({image , data , setLoading , getUserThreads  , id}){
    ...
    function handleBtnClick(){
        ...
        async     function addThread(){
                ...
                fullData.append('id' , data.id);
                
                
                if(img){
                    fullData.append('image' , img);
                    fullData.append('image_type' , img.name.slice(img.name.lastIndexOf('.') + 1));
                }
                ...
                if (content){
                    setLoading(true);
                    fullData.append('content' , content);
                    
                    axios.post(import.meta.env.VITE_API_URL + '/threads/store' , fullData , {
                        headers:{
                        ...
                        getUserThreads(id ? id : localStorage.token)
                        setContent('');
                        setFile(null);
                        setImg(null);
    ...
    return (
        <div className="contain">
            <div className="create fixed top-10 right-12  md:top-auto md:left-auto md:right-4 md:bottom-7 ">
                <div onClick= {handleBtnClick} className="btn cursor-pointer border border-black py-1 px-3 md:py-2 md:px-6 border-2  transition duration-300 hover:scale-105 flex justify-center items-center  shadow-black   rounded-3xl bg-gray-100">
                <i className="start_btn ph ph-plus font-bold text-3xl " ></i>
                </div>
            </div>
               ...
               <div className="head relative mb-3  flex  gap-6">
                    ...
                    <form onSubmit = {(e) => e.preventDefault()} className="edit_profile_form w-full flex flex-col gap-6">
                        ...
                        name="post_image" id="psi" className="hidden"/>
                            ...
                            {file && <div className="imageContainer mw-full ">
                                <img src={file} alt="" className="post_image_min mx-auto"/>
                            </div>}

                        <button onClick = {addThread} className="add bg-black text-gray-200 p-6 my-3 rounded-3xl">Create</button>
                    </form>
               </div>
            </div>
    }
